"""ホスト wrapper と Python runtime で共有する backend registry。"""
...
def get_backend_specs() -> tuple[BackendSpec, ...]:
    ...
    Returns:
        表示順の backend 定義。`MCON_BACKENDS` があれば、shell 側に
        metadata を増やさずに表示対象だけを絞る。
    """
...
def get_backend_spec(backend_name: str) -> BackendSpec:
    """backend 名または alias から backend 定義を探す。

    Args:
        backend_name: ユーザーが指定した backend 名。

    Returns:
        一致した backend 定義。

    Raises:
        ValueError: 未知の backend の場合。
    """
...
def get_model_candidates(backend_name: str) -> tuple[str, ...]:
    """backend の model 候補を標準 model 優先で返す。"""
...
def is_backend_authenticated(data_directory: Path, backend_name: str) -> bool:
    """backend が利用可能な認証情報を持っていそうか確認する。"""
...
def get_container_environment_names() -> tuple[str, ...]:
    """container に引き継ぐ環境変数名を registry から組み立てる。"""
